...
export class AuthorController {
  ...
  @Role(UserType.AUTHOR)
  @Get('visible-reviews')
@ApiOperation({ summary: 'Get approved reviews visible to logged-in author' })
getVisibleReviewsForAuthor(
  @User('userId') userId: string,
  @Query() query: FetchManuscriptDTO,
) {
  return this.authorService.getVisibleReviewsForAuthor(userId, query);
}
